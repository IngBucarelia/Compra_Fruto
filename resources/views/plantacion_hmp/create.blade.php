...
@section('content')
<div class="container" style="background-color: whitesmoke; border-radius:30px; padding:20px;width:85%">
...
@if($visita->aguaCaptacionLegal || $visita->aguaUsoEficiente || $visita->sueloConservacion || $visita->energiaManejo || $visita->gobernanzaHidrica || $visita->emisionesGei || $visita->residuosManejo || $visita->sustanciasQuimicasBiologicas || $visita->vertimientoManejo)
<div class="accordion mb-4 accordion-ambiental" id="accordionAmbiental">
    ...
    {{-- ACORDEÓN 9: VERTIMIENTO MANEJO (NUEVO) --}}
...
<div class="alert alert-info mb-4">
    ...
    <ul class="mb-0 mt-2">
        ...
        <li>
            ...
            <a href="{{ route('residuos_manejo.create', $visita->id) }}" class="btn btn-sm btn-outline-primary ms-2">
                Registrar ahora
            </a>
        </li>
        @endif
        ...
        <li>
            ...
            <a href="{{ route('vertimientos.create', $visita->id) }}" class="btn btn-sm btn-outline-primary ms-2">
                Registrar ahora
            </a>
        </li>
        @endif
...
<div class="alert alert-warning mb-4">
    ...
    <div class="mt-3">
        ...
        <a href="{{ route('residuos_manejo.create', $visita->id) }}" class="btn btn-danger me-2">
        ...
        <a href="{{ route('vertimientos.create', $visita->id) }}" class="btn btn-primary">
...
<div class="card-component">
    ...
    <form action="{{ route('plantacion_hmp.store') }}" method="POST" id="form-hmp">
    @csrf
    ...
    <table class="table table-bordered table-custom">
        <tr>
            ...
            <td>
                <select name="implementa_hmp" id="implementa_hmp" class="form-control" onchange="toggleHectareasHMP(this.value)">
        ...
        <!-- CAMPO QUE APARECE SOLO SI ES "Sí" -->
        <tr id="hectareas_hmp_row" style="display: none;">
            <th>
                <span class="text-success">🌿 Hectáreas con HMP implementadas</span>
                <br><small class="text-muted">Área total de la finca: {{ number_format($areaTotalFinca, 2) }} Ha</small>
            </th>
            <td>
                <div class="row">
                    <div class="col-md-6">
                        <div class="input-group">
                            <input type="number" 
                                   ...
                                   max="{{ $areaTotalFinca }}"
                                   placeholder="Ej: 10.5"
                                   oninput="calcularPorcentajeHMP()">
                            <span class="input-group-text">Ha</span>
                        </div>
                        <small class="text-muted">Hectáreas donde implementa HMP</small>
                    </div>
                    
                    <div class="col-md-6">
                        <div class="alert alert-info p-2">
                            <strong>Porcentaje:</strong>
                            <span id="porcentaje_hmp_display">0%</span>
                            <input type="hidden" name="porcentaje_hmp" id="porcentaje_hmp">
                        </div>
                    </div>
                </div>
                
                <!-- FÓRMULA VISUAL -->
                <div class="mt-2" id="formula_hmp" style="display: none;">
                    <small class="text-muted">
                        <i class="fas fa-calculator"></i> Fórmula: 
                        (<span id="hectareas_display">0</span> Ha ÷ {{ number_format($areaTotalFinca, 2) }} Ha) × 100 = 
                        <span id="porcentaje_calculado">0%</span>
                    </small>
                </div>
                ...
                <div class="alert alert-warning mt-2">
                    ...
                    No se encontró información del área total de la finca.
                </div>
                @endif
            </td>
        </tr>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const implementaHMP = document.getElementById('implementa_hmp');
    if (implementaHMP && implementaHMP.value === 'si') {
        toggleHectareasHMP('si');
    }
});

function toggleHectareasHMP(valor) {
    const row = document.getElementById('hectareas_hmp_row');
    const inputHectareas = document.getElementById('hectareas_hmp');
    const formulaDiv = document.getElementById('formula_hmp');
    
    if (valor === 'si') {
        row.style.display = 'table-row';
        inputHectareas.required = true;
        formulaDiv.style.display = 'block';
        calcularPorcentajeHMP();
    } else {
        row.style.display = 'none';
        inputHectareas.required = false;
        inputHectareas.value = '';
        formulaDiv.style.display = 'none';
        document.getElementById('porcentaje_hmp_display').textContent = '0%';
        document.getElementById('porcentaje_hmp').value = '';
    }
}

function calcularPorcentajeHMP() {
    const hectareasHMP = parseFloat(document.getElementById('hectareas_hmp').value) || 0;
    const areaTotalFinca = parseFloat(document.getElementById('area_total_finca').value) || 0;
    
    const porcentajeDisplay = document.getElementById('porcentaje_hmp_display');
    const porcentajeHidden = document.getElementById('porcentaje_hmp');
    const hectareasDisplay = document.getElementById('hectareas_display');
    const porcentajeCalculado = document.getElementById('porcentaje_calculado');
    
    if (areaTotalFinca > 0 && hectareasHMP > 0) {
        const porcentaje = (hectareasHMP / areaTotalFinca) * 100;
        
        // Validar que no sea mayor al área total
        if (hectareasHMP > areaTotalFinca) {
            porcentajeDisplay.textContent = '¡Error! Supera área total';
            porcentajeDisplay.className = 'text-danger';
            porcentajeHidden.value = '';
            return;
        }
        
        // Formatear valores
        hectareasDisplay.textContent = hectareasHMP.toFixed(2);
        porcentajeDisplay.textContent = porcentaje.toFixed(2) + '%';
        porcentajeCalculado.textContent = porcentaje.toFixed(2) + '%';
        porcentajeHidden.value = porcentaje.toFixed(2);
        
        // Color según porcentaje
        if (porcentaje >= 80) {
            porcentajeDisplay.className = 'text-success fw-bold';
        } else if (porcentaje >= 50) {
            porcentajeDisplay.className = 'text-warning fw-bold';
        } else {
            porcentajeDisplay.className = 'text-danger fw-bold';
        }
    } else {
        porcentajeDisplay.textContent = '0%';
        porcentajeDisplay.className = '';
        porcentajeHidden.value = '';
    }
}

// Validación antes de enviar
document.getElementById('form-hmp').addEventListener('submit', function(e) {
    const implementaHMP = document.getElementById('implementa_hmp').value;
    const areaTotalFinca = parseFloat(document.getElementById('area_total_finca').value) || 0;
    
    if (implementaHMP === 'si') {
        const hectareasHMP = document.getElementById('hectareas_hmp').value;
        
        // Validar que haya ingresado hectáreas
        if (!hectareasHMP.trim()) {
            e.preventDefault();
            alert('Por favor ingrese las hectáreas con HMP implementadas');
            document.getElementById('hectareas_hmp').focus();
            return false;
        }
        
        const hectareasNum = parseFloat(hectareasHMP);
        
        // Validar que no sea mayor al área total
        if (areaTotalFinca > 0 && hectareasNum > areaTotalFinca) {
            e.preventDefault();
            alert(`Las hectáreas con HMP (${hectareasNum} Ha) no pueden ser mayores al área total de la finca (${areaTotalFinca} Ha)`);
            document.getElementById('hectareas_hmp').focus();
            return false;
        }
        
        // Validar que haya área total para calcular porcentaje
        if (areaTotalFinca <= 0) {

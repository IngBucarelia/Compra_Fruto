...
@if($visita->aguaCaptacionLegal || $visita->aguaUsoEficiente || $visita->sueloConservacion || $visita->energiaManejo || $visita->gobernanzaHidrica || $visita->emisionesGei || $visita->residuosManejo || $visita->sustanciasQuimicasBiologicas)
<div class="accordion mb-4 accordion-ambiental" id="accordionAmbiental">
    ...
    {{-- ACORDEÓN 8: SUSTANCIAS QUIMICAS BIOLOGICAS (NUEVO) --}}
...
<div class="alert alert-warning mb-4">
    ...
    <div class="mt-3">
        ...
        <a href="{{ route('sustancias_quimicas_biologicas.create', $visita->id) }}" class="btn btn-purple">
...
{{-- REEMPLAZA tu formulario actual por este: --}}
<form action="{{ route('vertimientos.store') }}" method="POST" id="vertimientos-form">
    @csrf
    ...
    <div class="card-component">
        ...
        <select name="permiso_vertimientos"
    ...
    <div class="card-component" id="vertimientos-permiso-container" style="display:none;">
        <label>Número de vertimientos con permiso</label>
        <input type="number"
               name="numero_vertimientos_permitidos"
               min="0"
               class="form-control mb-2">

        <label>Número total de vertimientos</label>
        <input type="number"
               name="numero_vertimientos_totales"
               min="0"
               class="form-control">
    </div>
...
<div style="background: yellow; padding: 10px; margin: 10px 0;">
    ...
    Ruta: {{ route('vertimientos.store') }}<br>
    URL: {{ url('/vertimientos/store') }}<br>
    Método: POST<br>
    Visita ID: {{ $visitaId }}
</div>--}}
...
document.addEventListener('DOMContentLoaded', function () {
    const form = document.getElementById('vertimientos-form');
    const submitBtn = document.getElementById('submit-btn');
    const permisoSelect = document.getElementById('permiso_vertimientos');
    const container = document.getElementById('vertimientos-permiso-container');
    ...
    submitBtn.addEventListener('click', function(e) {
        ...
        camposRequeridos.forEach(campo => {
            ...
            if (!elemento.value) {
                valido = false;
                mensajesError.push(`El campo "${campo}" es requerido`);
        ...
        if (permisoSelect.value === '1') {
            const permitidos = form.querySelector('[name="numero_vertimientos_permitidos"]');
            const totales = form.querySelector('[name="numero_vertimientos_totales"]');
            
            if (!permitidos.value || !totales.value) {
                valido = false;
                mensajesError.push('Cuando tiene permiso, debe completar los campos numéricos');
            }
        }
    ...
    // DEBUG: Mostrar datos en consola al enviar

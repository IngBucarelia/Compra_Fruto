@extends('layouts.app')

@section('content')
<div class="members-wrap">
    <div class="container offline-form-container" style="background-color: #e8d5dce0; ">
        {{-- Botón dinámico según estado --}}
    @if ($visita->estado !== 'finalizada')
        <form action="{{ route('redireccion_seccion_social', $visita->id) }}" method="GET" class="mt-4">
            <label for="seccion" class="form-label fw-bold text-success">📋 Ir a sección:</label>
            <div class="input-group">
                <select id="seccion" name="seccion" class="form-select" required>
                    <option value="">Seleccione una sección</option>
                    @if ($visita->estado === 'pendiente' || $visita->estado === 'en_ejecucion')
                        <option value="inicio"> Pagina de Inicio de Visita</option>
                        <option value="datos_personales">👤 Datos Personales</option>
                        <option value="miembros">👨‍👩‍👧‍👦 Miembros del Hogar</option>
                        <option value="predio">🏡 Datos del Predio</option>
                        <option value="fuerza_laboral">🧑‍🌾 Fuerza Laboral</option>
                        <option value="organizacion_social">👥 Organización Social</option>
                    @endif
                </select>
                <button type="submit" class="btn btn-success">Ir</button>
            </div>
        </form>
    @endif
        <!-- Título -->
        <h2 class="members-title text-center">
            🧑‍🌾 Datos Personales del Productor
        </h2>

        <!-- Mensaje de éxito -->
        @if(session('success'))
            <div class="alert success">{{ session('success') }}</div>
        @endif

        <!-- Botón hacia miembros -->
        <div class="mb-4">
            <a href="{{ route('miembros_hogar.index', $visita->id) }}" class="btn btn-primary">
                👨‍👩‍👧‍👦 Ir a Miembros del Hogar
            </a>
        </div>

        <!-- Tabla de datos -->
        <div class="table-responsive">
            <table class="custom-table">
                <tr><th>📞 Teléfono</th><td>{{ $datos->telefono }}</td></tr>
                <tr><th>⚧ Sexo</th><td>{{ $datos->sexo }}</td></tr>
                <tr><th>🆔 RNP</th><td>{{ $datos->rnp }}</td></tr>
                <tr><th>🌴 Afiliado Fedepalma</th><td>{{ $datos->fedepalma }}</td></tr>
                <tr><th>📖 Alfabetizado</th><td>{{ $datos->alfabetizado }}</td></tr>
                <tr><th>🎓 Nivel de estudio</th><td>{{ $datos->nivel_estudio }}</td></tr>
                <tr><th>💼 Otras líneas</th><td>{{ $datos->otras_lineas }}</td></tr>
                <tr><th>🎂 Fecha nacimiento</th><td>{{ $datos->fecha_nacimiento }}</td></tr>
                <tr><th>👥 Grupo poblacional</th><td>{{ $datos->grupo_poblacional }}</td></tr>
                <tr><th>🏠 Reside en predio</th><td>{{ $datos->reside_predio }}</td></tr>
                <tr><th>🌱 Administra cultivo</th><td>{{ $datos->administra_cultivo }}</td></tr>
                <tr><th>🔎 Supervisa cultivo</th><td>{{ $datos->supervisa_cultivo }}</td></tr>
                <tr><th>🛠️ Realiza cultivo</th><td>{{ $datos->realiza_cultivo }}</td></tr>
                <tr><th>⏳ Años en palmicultura</th><td>{{ $datos->anios_palmicultura }}</td></tr>
                <tr><th>🌐 Internet</th><td>{{ $datos->internet }}</td></tr>
                <tr><th>👤 Tipo persona</th><td>{{ $datos->tipo_persona }}</td></tr>
                <tr><th>📱 Red social</th><td>{{ $datos->red_social }}</td></tr>
                <tr><th>🏥 Régimen salud</th><td>{{ $datos->regimen_salud }}</td></tr>
            </table>
        </div>

        <!-- Botones -->
        <div class="form-actions mt-4">
            <a href="{{ route('visitas_social.showSocial', $visita->id) }}" class="btn btn-ghost">⬅ Volver</a>
            <a href="{{ route('datos_personales_sociales.edit', $visita->id) }}?id={{ $datos->id }}" 
            class="btn btn-warning">
            ✏️ Editar
            </a>

        </div>
    </div>
</div>

<!-- Estilos locales -->
<style>
.members-wrap {
    max-width: 125%;
    border-radius: 5px;
}
.members-card {
     max-width: 125%;
    background: #fff;
    border-radius: 22px;
    padding: 24px;
    box-shadow: 0 8px 30px rgba(12, 50, 20, 0.08);
}
.members-title {
    font-size: 1.6rem;
    color: #19692b;
    font-weight: 700;
    margin-bottom: 20px;
}
.alert.success {
    background: #e9f7ee;
    color: #19692b;
    padding: 10px 14px;
    border-radius: 8px;
    margin-bottom: 16px;
    font-weight: 600;
}
.custom-table {
    width: 100%;
    border-collapse: collapse;
    background: #fff;
    border-radius: 8px;
    overflow: hidden;
}
.custom-table th {
    background: #f0fdf4;
    color: #14532d;
    text-align: left;
    padding: 10px 14px;
    width: 40%;
    font-weight: 600;
    border-bottom: 1px solid #e5e7eb;
}
.custom-table td {
    padding: 10px 14px;
    border-bottom: 1px solid #e5e7eb;
    color: #374151;
}
.custom-table tr:hover td {
    background: #f9fafb;
}
.form-actions {
    display: flex;
    justify-content: space-between;
    align-items: center;
    gap: 10px;
}
.btn {
    display: inline-block;
    padding: 10px 18px;
    border-radius: 8px;
    font-weight: 600;
    font-size: 0.95rem;
    text-decoration: none;
    cursor: pointer;
    text-align: center;
}
.btn-primary {
    background: #198754;
    color: #fff;
    border: none;
}
.btn-primary:hover {
    background: #14673f;
}
.btn-ghost {
    background: #f3f4f6;
    color: #374151;
}
.btn-ghost:hover {
    background: #e5e7eb;
}
.btn-warning {
    background: #f59e0b;
    color: #fff;
}
.btn-warning:hover {
    background: #d97706;
}
   @media (max-width: 968px) {

         .container.offline-form-container {
        background-color: rgba(129, 165, 114, 0.929); /* Color de fondo específico para este formulario */
    }
        .button-group-top {
            flex-direction: row;
            justify-content: flex-start;
        }

         .container.offline-form-container {
        padding: 15px;
            margin-top: 15px;
            border-radius: 0;
            box-shadow: none;
            width: 123%;
            max-width: none;
            margin-left: -60px !important;
    }

    .title{
    text-align: center;
    font-family: Arial Black;
    font-weight: bold;
    font-size: 30px;
    color: #fdffe5;
    text-shadow: -1px 0 #000, 0 1px #000, 1px 0 #000, 0 -1px #000;
    margin-bottom: 25px;
}

 .container {
        margin-left: -70px;
        width: 125%;
    

    }

        .dashboard-content {
            max-width: 100%;
        }
        .dashboard-card {
            margin-bottom: 15px;
        }

        .card{
        width: 100%;
    }
    }
</style>
@endsection
